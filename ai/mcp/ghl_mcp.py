...
ai/mcp/ghl_mcp.py
------------------
The GoHighLevel (GHL) CRM tools (get_ghl_contacts, get_ghl_opportunity_stats,
get_ghl_opp_stats_windowed, get_ghl_opp_stats_monthly, get_ghl_tag_breakdown,
get_tag_rollup_by_campaign), registered onto the shared FastMCP server
(ai/mcp/server.py) served over the Model Context Protocol, mounted into the
main FastAPI app at /mcp (see main.py). This is a real MCP server — reachable
by external MCP clients (Claude Desktop, Claude Code, etc.), not just Birdy's
own orchestrator.

Business logic here is a straight port of ai/tools/ghl_tools.py — see that
...
_GET_GHL_CONTACTS_DESCRIPTION = (
    "Get GoHighLevel CRM contacts with name, email, phone, tags, source, and opportunity data. "
...
@mcp.tool(description=_GET_GHL_CONTACTS_DESCRIPTION)
...
_GET_GHL_OPPORTUNITY_STATS_DESCRIPTION = (
    ...
    "Args:\n"
    f"    preset: Date range preset. Valid values: {_PRESET_LIST}. Default: maximum (lifetime).\n"
    "    group_ids: List of client group IDs to filter by. Omit for all groups."
)


@mcp.tool(description=_GET_GHL_OPPORTUNITY_STATS_DESCRIPTION)
async def get_ghl_opportunity_stats(
    preset: str = "maximum",
    group_ids: list[str] | None = None,
) -> dict:
    """
...
_GET_GHL_TAG_BREAKDOWN_DESCRIPTION = (
    ...
    "Args:\n"
    f"    preset: Date range preset. Valid values: {_PRESET_LIST}. Default: maximum.\n"
    "    group_ids: List of client group IDs to filter by. Omit for all groups.\n"
    "    top_n: Number of top tags to return per group (default 20)."
)


@mcp.tool(description=_GET_GHL_TAG_BREAKDOWN_DESCRIPTION)
async def get_ghl_tag_breakdown(
    preset: str = "maximum",
    group_ids: list[str] | None = None,
    top_n: int = 20,
) -> dict:
...
_GET_TAG_ROLLUP_BY_CAMPAIGN_DESCRIPTION = (
    ...
    "Args:\n"
    "    level: Roll up level: 'campaign', 'adset', or 'ad'. Default: campaign.\n"
    "    group_ids: List of client group IDs to filter by. Omit for all groups."
)


@mcp.tool(description=_GET_TAG_ROLLUP_BY_CAMPAIGN_DESCRIPTION)
async def get_tag_rollup_by_campaign(
    group_ids: list[str] | None = None,
    level: str = "campaign",
) -> dict:
    """
...
_GET_GHL_OPP_STATS_WINDOWED_DESCRIPTION = (
    ...
    "Args:\n"
    "    group_id: The client group ID (from get_client_groups).\n"
    "    start_date: Window start in YYYY-MM-DD format.\n"
    "    end_date: Window end in YYYY-MM-DD format."
)


@mcp.tool(description=_GET_GHL_OPP_STATS_WINDOWED_DESCRIPTION)
async def get_ghl_opp_stats_windowed(
    group_id: str,
    start_date: str | None = None,
    end_date: str | None = None,
) -> dict:
...
_GET_GHL_OPP_STATS_MONTHLY_DESCRIPTION = (
...
@mcp.tool(description=_GET_GHL_OPP_STATS_MONTHLY_DESCRIPTION)
async def get_ghl_opp_stats_monthly(group_id: str, year: int) -> dict:
    user_id = _current_user_id()
    db = get_db(get_shared_mongo_client())
