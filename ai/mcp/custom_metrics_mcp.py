...
-----------------------------
The custom metrics tools (list_custom_metrics, compute_custom_metric,
list_available_metric_fields, create_custom_metric, update_custom_metric,
delete_custom_metric), registered onto the shared FastMCP server
...
Business logic here is a straight port of ai/tools/custom_metrics_tools.py —
see that module for the (unchanged, still-registered) fallback path used by
the orchestrator if the MCP path is unavailable. All metric resolution and
formula evaluation is delegated to services.metric_orchestrator so logic
stays in one place, exactly as in the original module.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Formula parsing (unchanged from ai/tools/custom_metrics_tools.py)
# ─────────────────────────────────────────────────────────────────────────────
...
_LIST_CUSTOM_METRICS_DESCRIPTION = (
...
@mcp.tool(description=_LIST_CUSTOM_METRICS_DESCRIPTION)
async def list_custom_metrics() -> dict:
...
_COMPUTE_CUSTOM_METRIC_DESCRIPTION = (
...
@mcp.tool(description=_COMPUTE_CUSTOM_METRIC_DESCRIPTION)
async def compute_custom_metric(
    ...
    """
    Args:
        metric_id: The custom metric ID (from list_custom_metrics)
        preset: Date preset (maximum, today, last_7d, last_30d, this_month, etc.). Default: last_7d.
...
_LIST_AVAILABLE_METRIC_FIELDS_DESCRIPTION = (
...
@mcp.tool(description=_LIST_AVAILABLE_METRIC_FIELDS_DESCRIPTION)
async def list_available_metric_fields() -> dict:
    metrics = []
    labels = {
...
_CREATE_CUSTOM_METRIC_DESCRIPTION = (
...
@mcp.tool(description=_CREATE_CUSTOM_METRIC_DESCRIPTION)
async def create_custom_metric(
    name: str,
    formula_text: str | None = None,
    formula_parts: list[dict] | None = None,
    ...
    Args:
        ...
        formula_text: Friendly formula string (e.g. 'meta_spend / ghl_revenue'). Operators: + - * /
        formula_parts: Structured formula (alternative to formula_text). Each item is
            {"type": "metric"|"operator", "value": str}.
        description: Optional longer description
        format_type: How to display the result. Valid: integer, currency, percentage, decimal. Default: integer
        dashboards: Where the metric appears. Valid: clients, campaigns, adsets, ads, leads, marketing_leads.
            Omit to auto-detect based on formula.
        aggregation: How to aggregate across groups. Valid: total, average. 'average' triggers ratio
            recomputation (recommended for percentage/ratio metrics).
    """
    user_id = _current_user_id()
    db = get_db(get_shared_mongo_client())
...
_UPDATE_CUSTOM_METRIC_DESCRIPTION = (
...
@mcp.tool(description=_UPDATE_CUSTOM_METRIC_DESCRIPTION)
async def update_custom_metric(
    ...
    Args:
        ...
        formula_text: New formula as a friendly string
        formula_parts: New structured formula (alternative to formula_text). Each item is
            {"type": "metric"|"operator", "value": str}.
        description: New description
        format_type: New format type. Valid: integer, currency, percentage, decimal.
        dashboards: New dashboards. Valid: clients, campaigns, adsets, ads, leads, marketing_leads.
        aggregation: New aggregation. Valid: total, average.
    """
    user_id = _current_user_id()
    db = get_db(get_shared_mongo_client())
...
_DELETE_CUSTOM_METRIC_DESCRIPTION = (
    "Delete a custom metric by id. Confirm with the user before calling — deletion cannot be undone."
)


@mcp.tool(description=_DELETE_CUSTOM_METRIC_DESCRIPTION)
async def delete_custom_metric(metric_id: str) -> dict:
    """
    Args:
