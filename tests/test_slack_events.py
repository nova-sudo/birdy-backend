...
async def test_app_mention_strips_mention_prefix_and_calls_run_chat(signed_post):
    ...
    post_kwargs = mock_post_message.call_args.kwargs
    assert post_kwargs["channel"] == "C1"
    assert post_kwargs["thread_ts"] == "1111.2222"
    assert "The answer is 42." in post_kwargs["text"]
...
async def test_no_ai_credential_posts_helpful_message_instead_of_erroring(signed_post):
    ...
    assert resp.status_code == 200
    mock_post_message.assert_awaited_once()
    assert "connect" in mock_post_message.call_args.kwargs["text"].lower()
